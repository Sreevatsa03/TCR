...
int main(int argc, char** argv)
{
	...
	float diff3 = (float)t6 - (float)t5;
        cout << "Total time for conversion is = " << diff3/CLOCKS_PER_SEC << " seconds\n";
	cudaProfilerStart();
	...
	if(N == 12)
		...
		result_Memsize = (numCombN/128) * VJ_Largest * sizeof(unsigned int); 
	else if ( (N > 2) && (N < 6) )
...
threads_per_block(int n_val){
	...
	switch(n_val){
		   ...
		   case 7:threads =  128; break;
		   case 8:threads =  128; break;
		   case 9:threads =  128; break;
		   case 10:threads = 128; break;
